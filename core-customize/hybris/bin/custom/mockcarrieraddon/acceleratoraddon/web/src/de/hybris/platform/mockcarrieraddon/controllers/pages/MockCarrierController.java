 ...
 */
package de.hybris.platform.mockcarrieraddon.controllers.pages;

import de.hybris.platform.basecommerce.enums.ConsignmentStatus;
import de.hybris.platform.mockcarrieraddon.bean.ConsignmentPojo;
import de.hybris.platform.mockcarrieraddon.controllers.ConsignmenttrackingmockaddonControllerConstants;
import de.hybris.platform.mockcarrieraddon.data.MockDataProvider;
import de.hybris.platform.mockcarrieraddon.forms.TrackingEventForm;
...
public class MockCarrierController
{
	...
	@RequestMapping(value = "/consignmentDetail", method = RequestMethod.GET)
	public @ResponseBody ConsignmentPojo displayAjax(final HttpServletRequest request, final HttpServletResponse response)
	{
		final ConsignmentPojo consignmentdetails = new ConsignmentPojo();
		...
		mockDataProvider.getConsignmentForTrackingId(carrier, trackingId).ifPresent(consignment -> {
			...
			final Double price = consignment.getOrder().getTotalPrice();
			final String sku = consignment.getOrder().getEntries().get(0).getProduct().getCode();
			final String productName = consignment.getOrder().getEntries().get(0).getProduct().getName();

			consignmentdetails.setConsignmentCode(consignmentCode);
			consignmentdetails.setOrderCode(orderCode);
			consignmentdetails.setPrice(price);
			consignmentdetails.setProductName(productName);
			consignmentdetails.setSku(sku);
		});
