...
public class OrderCancelRepairJobTest
{
	...
	public void cronJobTest()
	{
		...
		when(orderExchangeRepair.findProcessesByActionIds(SapphiresaporderfulfillmentConstants.ORDER_PROCESS_NAME, actions)).thenReturn(
				bpList);
		repairJob.perform(cronJobModel);
		verify(orderExchangeRepair).findProcessesByActionIds(SapphiresaporderfulfillmentConstants.ORDER_PROCESS_NAME, actions);
	}
