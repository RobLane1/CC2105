...
public class AccountSupportTicketsPageController extends AbstractSearchPageController
{
	...
	{
		storeCmsPageInModel(model, getContentPageForLabelOrId(SapphirecustomerticketaddonConstants.SUPPORT_TICKETS_PAGE));
		setUpMetaDataForContentPage(model, getContentPageForLabelOrId(SapphirecustomerticketaddonConstants.SUPPORT_TICKETS_PAGE));
		model.addAttribute(WebConstants.BREADCRUMBS_KEY,
				accountBreadcrumbBuilder.getBreadcrumbs(SapphirecustomerticketaddonConstants.TEXT_SUPPORT_TICKETING_HISTORY));
		...
		if (ticketAdded)
		{
			GlobalMessages.addConfMessage(model, SapphirecustomerticketaddonConstants.TEXT_SUPPORT_TICKETING_ADDED);
		}
	...
	{
		storeCmsPageInModel(model, getContentPageForLabelOrId(SapphirecustomerticketaddonConstants.ADD_SUPPORT_TICKET_PAGE));
		setUpMetaDataForContentPage(model, getContentPageForLabelOrId(SapphirecustomerticketaddonConstants.ADD_SUPPORT_TICKET_PAGE));

		model.addAttribute(WebConstants.BREADCRUMBS_KEY,
				getBreadcrumbs(SapphirecustomerticketaddonConstants.TEXT_SUPPORT_TICKETING_ADD));
		model.addAttribute(ThirdPartyConstants.SeoRobots.META_ROBOTS, ThirdPartyConstants.SeoRobots.NOINDEX_NOFOLLOW);
		model.addAttribute(SapphirecustomerticketaddonConstants.SUPPORT_TICKET_FORM, new SupportTicketForm());
		model.addAttribute(SapphirecustomerticketaddonConstants.MAX_UPLOAD_SIZE, Long.valueOf(maxUploadSizeValue));
		model.addAttribute(SapphirecustomerticketaddonConstants.MAX_UPLOAD_SIZE_MB,
				FileUtils.byteCountToDisplaySize(maxUploadSizeValue));
		try
		{
			model.addAttribute(SapphirecustomerticketaddonConstants.SUPPORT_TICKET_ASSOCIATED_OBJECTS,
					ticketFacade.getAssociatedToObjects());
			model.addAttribute(SapphirecustomerticketaddonConstants.SUPPORT_TICKET_CATEGORIES, ticketFacade.getTicketCategories());
			
			if(ticketFacade instanceof DemoC4CTicketFacadeImpl){
				model.addAttribute("products", ((DemoC4CTicketFacadeImpl)ticketFacade).getAssociatedProducts());
			}
		}
			...
			final BindingResult bindingResult)
	{
		...
		if (bindingResult.hasErrors())
		{
			...
			list.add(buildMessageMap(SapphirecustomerticketaddonConstants.FORM_GLOBAL_ERROR_KEY,
					SapphirecustomerticketaddonConstants.FORM_GLOBAL_ERROR));
		...
		catch (final UnsupportedAttachmentException usAttEx)
		{
			...
			map.put(SapphirecustomerticketaddonConstants.SUPPORT_TICKET_TRY_LATER, getMessageSource()
					.getMessage(SapphirecustomerticketaddonConstants.TEXT_SUPPORT_TICKETING_ATTACHMENT_BLOCK_LISTED, new Object[]
		...
		catch (final RuntimeException rEX)
		{
			...
			map.put(SapphirecustomerticketaddonConstants.SUPPORT_TICKET_TRY_LATER, getMessageSource().getMessage(
					SapphirecustomerticketaddonConstants.TEXT_SUPPORT_TICKETING_TRY_LATER, null, getI18nService().getCurrentLocale()));
			return new ResponseEntity<>(Lists.newArrayList(map), HttpStatus.BAD_REQUEST);
		}
	...
	{
		storeCmsPageInModel(model, getContentPageForLabelOrId(SapphirecustomerticketaddonConstants.UPDATE_SUPPORT_TICKET_PAGE));
		setUpMetaDataForContentPage(model, getContentPageForLabelOrId(SapphirecustomerticketaddonConstants.UPDATE_SUPPORT_TICKET_PAGE));

		model.addAttribute(WebConstants.BREADCRUMBS_KEY,
				getBreadcrumbs(SapphirecustomerticketaddonConstants.TEXT_SUPPORT_TICKETING_UPDATE));
		model.addAttribute(ThirdPartyConstants.SeoRobots.META_ROBOTS, ThirdPartyConstants.SeoRobots.NOINDEX_NOFOLLOW);
		model.addAttribute(SapphirecustomerticketaddonConstants.SUPPORT_TICKET_FORM, new SupportTicketForm());
		model.addAttribute(SapphirecustomerticketaddonConstants.MAX_UPLOAD_SIZE, Long.valueOf(maxUploadSizeValue));
		model.addAttribute(SapphirecustomerticketaddonConstants.MAX_UPLOAD_SIZE_MB,
		...
		try
		{
			...
			model.addAttribute(SapphirecustomerticketaddonConstants.SUPPORT_TICKET_DATA, ticketData);
		}
		catch (final Exception e)
		{
			LOG.error("Attempted to load ticket details that does not exist or is not visible", e);
			GlobalMessages.addFlashMessage(redirectModel, GlobalMessages.ERROR_MESSAGES_HOLDER,
					SapphirecustomerticketaddonConstants.TEXT_SUPPORT_TICKETING_TRY_LATER, null);
	...
	protected TicketData populateTicketData(final SupportTicketForm supportTicketForm) // TODO: should be moved to populator class
	{
		...
		ticketData.setObjectID(supportTicketForm.getObjectID());
		return ticketData;
	}
	...
	protected List<Breadcrumb> getBreadcrumbs(final String breadcrumbCode)
	{
		...
		breadcrumbs.add(new Breadcrumb("/my-account/support-tickets", getMessageSource().getMessage(
				SapphirecustomerticketaddonConstants.TEXT_SUPPORT_TICKETING_HISTORY, null, getI18nService().getCurrentLocale()), null));
		breadcrumbs.add(
