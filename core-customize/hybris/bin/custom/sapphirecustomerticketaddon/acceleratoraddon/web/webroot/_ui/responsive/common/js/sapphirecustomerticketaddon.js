ACC.sapphirecustomerticketaddon = {
	...
	onStatusChange: function () {
        $(document).on('change', '.js-add-message-status', function () {
            ACC.sapphirecustomerticketaddon.disableMessage(this);
		});
	},

    onFileChosen: function () {
        $(document).on('change', '#supportTicketForm input[name=files]', function () {
            ACC.sapphirecustomerticketaddon.clearAlerts();
            var selectedFile = document.getElementById('attachmentFiles');
            if (!ACC.sapphirecustomerticketaddon.isSelectedFilesValid(selectedFile))
            {
	...
	postNewMessage: function () {
		...
		$('.ct-add-new-msg-btn').on('click touchstart', function(e) {
			...
			$.colorbox({
                ...
                onComplete: function () {                	
                    ACC.sapphirecustomerticketaddon.disableMessage($('.js-add-message-status'));
    ...
    bindTicketAddActions: function () {
        $(document).on('click', '#addTicket',
            function (event) {
                ...
                ACC.sapphirecustomerticketaddon.formPostAction("support-tickets?ticketAdded=true");
            });
    },
    ...
    bindTicketUpdateActions: function () {
        $(document).on('click', '#updateTicket',
            function (event) {        	
                ...
                ACC.sapphirecustomerticketaddon.formPostAction('?ticketUpdated=true');
            });
    },
    ...
    formPostAction: function (successRedirectUrl) {
        ...
        var selectedFile = document.getElementById('attachmentFiles');
        if (!ACC.sapphirecustomerticketaddon.isSelectedFilesValid(selectedFile)) {
            ACC.sapphirecustomerticketaddon.displayCustomerTicketingAlert({
                type: 'error',
        ...
        $.ajax({
            ...
            error: function (jqXHR) {
                ACC.sapphirecustomerticketaddon.processErrorResponse(jqXHR);
            }
        });
    },
    ...
    processErrorResponse: function (jqXHR) {
            ...
            $.each(jqXHR.responseJSON, function() {
                $.each(this, function(k, v) {
                    ...
                    if (k === 'NotEmpty-supportTicketForm-subject'
                        ...
                        ACC.sapphirecustomerticketaddon.addHasErrorClass();
                    }
                    else {
                        ACC.sapphirecustomerticketaddon.displayGlobalAlert({type: 'error', message: v});
                    }
                });
            });
        ...
        ACC.sapphirecustomerticketaddon.displayCustomerTicketingAlert({type: 'error', messageId: 'supporttickets-tryLater'});
    },
