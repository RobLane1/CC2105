 ...
 */
package sapphire.hcl.com.bulkorderupload.action;
...
public class BulkCartFromUploadFileAction
{
	private static final Logger LOG = Logger.getLogger(BulkCartFromUploadFileAction.class);
	...
	public CartModel executeAction(final SavedCartFileUploadProcessModel cartFileUploadProcessModel) throws RetryLaterException
	{
		if (LOG.isDebugEnabled())
		{
			LOG.debug(
					"Started SavedCartFromUploadFileAction for SavedCartFileUploadProcess:" + cartFileUploadProcessModel.getCode());
		}
		final SavedCartFileUploadReportData savedCartFileUploadReportData = getImpersonationService().executeInContext(
				...
				new ImpersonationService.Executor<SavedCartFileUploadReportData, ImpersonationService.Nothing>()
				{
					...
					public SavedCartFileUploadReportData execute() throws ImpersonationService.Nothing
					{
						...
						try
						{
							return getSavedCartFileUploadStrategy().createSavedCartFromFile(cartFileUploadProcessModel.getUploadedFile(),
									savedCart);
						}
		...
		getModelService().refresh(savedCart);
		return savedCart;
	}
			...
			final SavedCartFileUploadProcessModel savedCartFileUploadProcessModel)
	{
		...
		{
			final String productMsg = (commerceCartModification.getEntry() != null)
					? commerceCartModification.getEntry().getProduct().getCode() : "";
			logMsgBuilder.append(getL10NService().getLocalizedString(getCannotImportErrorMessageKey(), new String[]
			{ productMsg }));
